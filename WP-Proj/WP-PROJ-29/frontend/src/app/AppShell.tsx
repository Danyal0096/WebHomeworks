import { Bell, CircleHelp, Headphones, Home, Library, LogOut, Menu, Search, Settings, ShieldCheck, SlidersHorizontal, Sparkles, TicketCheck, UserRound, Users, X } from "lucide-react";
...
export function AppShell() {
  const { t, i18n } = useTranslation(); const user = useSession()!; useDatabaseVersion(); const navigate = useNavigate();
  const [moreOpen, setMoreOpen] = useState(false); const [logoutOpen, setLogoutOpen] = useState(false);
  ...
  const openLogout = () => { setMoreOpen(false); setLogoutOpen(true); };
  const confirmLogout = () => {
    sessionStorage.setItem("sonora:logout-toast", "loggedOutToast");
    repository.logout();
    navigate("/login", { replace: true, state: { toastKey: "loggedOutToast" } });
  };
  const accountInitial = (user.artistProfile?.stageName ?? user.displayName).slice(0, 1).toUpperCase();
  const accountCard = () => <div className="account-card" aria-label={t("accountControl")}>{user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : <span>{accountInitial}</span>}<div><small>{t("signedInAs")}</small><strong>{user.artistProfile?.stageName ?? user.displayName}</strong><em>{user.email}</em></div></div>;
  return <div className="app-shell">
    <aside className="sidebar"><Logo />{!isStaff && <nav aria-label={t("primaryNavigation")}><NavItem to="/" icon={Home} label={t("home")} /><NavItem to="/search" icon={Search} label={t("search")} /><NavItem to="/library" icon={Library} label={t("library")} /><NavItem to="/notifications" icon={Bell} label={t("notifications")} badge={unread} /><NavItem to="/rooms" icon={Users} label={t("groupListening")} /></nav>}
