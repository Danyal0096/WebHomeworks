...
export function LibraryPage() {
  ...
  return <div className="page"><header className="page-heading with-action"><div><span className="eyebrow">{t("library")}</span><h1>{t("yourLibrary")}</h1></div><button className="button primary" onClick={() => setCreating(true)}><Plus />{t("createPlaylist")}</button></header>
    ...
    <Section title={t("recentHistory")}><div id="history" className="track-list">{recent.length ? recent.map((track, index) => <TrackRow key={track.id} track={track} context={recent} index={index} compact />) : <EmptyState icon={Clock3} title={t("historyEmpty")} body={t("historyEmptyBody")} />}</div></Section>
