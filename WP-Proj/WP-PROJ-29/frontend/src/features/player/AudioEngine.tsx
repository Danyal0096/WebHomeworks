...
export function AudioEngine() {
  ...
  useEffect(() => {
    ...
    counted.current = null; element.currentTime = 0; state.setProgress(0, track.durationSeconds);
  ...
  return <audio
    ...
    onTimeUpdate={(event) => {
      ...
      if (element.currentTime >= threshold && counted.current !== track.id && track.isPlayableForViewer) { counted.current = track.id; repository.recordValidStream(track.id); }
    }}
    ...
    onError={() => handleFailure(track.id)}
    ...
    onPlaying={() => { if (stallTimer.current) clearTimeout(stallTimer.current); stallTimer.current = null; }}
  />;
}
