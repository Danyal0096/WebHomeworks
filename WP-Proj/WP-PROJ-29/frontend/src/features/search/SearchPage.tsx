...
export function SearchPage() {
  ...
  return <div className="page search-page"><header className="page-heading"><span className="eyebrow">{t("search")}</span><h1>{t("searchTitle")}</h1></header><div className="search-box"><SearchIcon /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("searchPlaceholder")} /></div><div className="filter-row"><select value={sort} onChange={(e) => setSort(e.target.value as typeof sort)} aria-label={t("search")}><option value="popular">{t("sortPopular")}</option><option value="newest">{t("sortNewest")}</option></select><select value={genre} onChange={(e) => setGenre(e.target.value)} aria-label={t("genre")}><option value="all">{t("allGenres")}</option>{genres.map((item) => <option key={item}>{item}</option>)}</select></div>
