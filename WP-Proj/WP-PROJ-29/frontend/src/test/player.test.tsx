...
describe("queue and player behavior", () => {
  beforeEach(() => {
    repository.reset(); repository.login("listener.gold@sonora.demo", DEMO_PASSWORD);
    usePlayer.setState({ trackIds: [], currentIndex: -1, isPlaying: false, position: 0, repeatMode: "off", shuffleEnabled: false, failureCount: 0, unavailableIds: [], toast: null });
  });

  it("replaces the queue with the selected playback context", () => {
    const tracks = repository.tracks().slice(0, 4); act(() => usePlayer.getState().replaceContext(tracks, tracks[2].id)); expect(usePlayer.getState().trackIds).toEqual(tracks.map((track) => track.id)); expect(usePlayer.getState().currentIndex).toBe(2); expect(usePlayer.getState().isPlaying).toBe(true);
  });
  ...
  it("supports keyboard play/pause and queue shortcuts", () => {
    const tracks = repository.tracks().slice(0, 2); act(() => usePlayer.getState().replaceContext(tracks, tracks[0].id)); const view = render(<MemoryRouter><Player /></MemoryRouter>); fireEvent.keyDown(window, { code: "Space" }); expect(usePlayer.getState().isPlaying).toBe(false); fireEvent.keyDown(window, { key: "q" }); expect(usePlayer.getState().queueOpen).toBe(true); view.unmount();
  });
