...
describe("critical UI flows", () => {
  ...
  it("renders exact locale labels and rejects mojibake in UI dictionaries", () => {
    ...
    const labels = ["English", "Español", "Deutsch", "Français", "Русский", "简体中文"];
    render(<MemoryRouter initialEntries={["/register"]}><App /></MemoryRouter>);
    expect(screen.getAllByRole("option").map((option) => option.textContent).filter((text) => labels.includes(text ?? ""))).toEqual(labels);
    ...
    ["en", "es", "de", "fr", "ru", "zh-CN"].forEach((language) => {
      ...
      Object.entries(bundle).forEach(([key, value]) => {
        if (typeof value === "string") expect(value, `${language}.${key}`).not.toMatch(suspicious);
      });
    });
  });

  it("translates high-risk UI areas without falling back to English copy", () => {
    const keys = ["settingsTitle", "checkoutTitle", "profileImageTitle", "notificationTitle", "ticketGate", "admin", "studio", "artist", "likedEmpty", "historyEmpty", "playerError", "logoutConfirmTitle", "loggedOutToast"];
  ...
  it("does not expose raw internal API error codes in user-facing errors", () => {
    expect(uiError(new RepositoryError("api_error", "Request failed."), i18n.t)).toBe("The request could not be completed.");
    expect(uiError(new RepositoryError("api_unavailable", "Bad gateway."), i18n.t)).toBe("The Sonora API is not responding correctly.");
    expect(uiError(new RepositoryError("network_error", "Failed to fetch."), i18n.t)).toBe("Sonora cannot reach the server. Check that the backend is running.");
    expect(uiError(new RepositoryError("unknown_internal_code", "traceback-ish"), i18n.t)).toBe("Something went wrong");
  });

  it("keeps integrated media and API repository boundaries explicit", () => {
    const viteConfig = readFileSync(join(process.cwd(), "vite.config.ts"), "utf8");
    const apiRepository = readFileSync(join(process.cwd(), "src", "repositories", "apiRepository.ts"), "utf8");
    expect(viteConfig).toContain('"/media":');
    expect(viteConfig).toContain("http://127.0.0.1:8000");
    expect(apiRepository).toContain('request<Payment>("/subscription/purchases/"');
    ...
    expect(apiRepository).toContain('throw new RepositoryError("backend_upload_required"');
    const trackRow = readFileSync(join(process.cwd(), "src", "components", "TrackRow.tsx"), "utf8");
    const serializers = readFileSync(join(process.cwd(), "..", "backend", "api", "serializers.py"), "utf8");
  ...
  it("does not ship integrated checkout copy that claims local demo storage", () => {
    ...
    expect(bundle).not.toMatch(/DEMO SIMULATION|local demo storage|local demo checkout|selected paid tier immediately in local/i);
  });

  it("uses the vinyl cover fallback only when a cover is intentionally absent", () => {
    const { container, rerender } = render(<CoverArt src={null} alt="Missing cover" />);
    expect(container.querySelector("img")).toBeNull();
    expect(screen.getByRole("img", { name: "Missing cover" })).toHaveClass("vinyl");
    rerender(<CoverArt src="/media/covers/release-1.svg" alt="Seed cover" />);
    expect(container.querySelector("img")).toHaveAttribute("src", "/media/covers/release-1.svg");
  });
  ...
  it("keeps search focus to one premium treatment without the old double lime halo", () => {
    ...
    expect(css).toContain(".search-box input:focus,.search-box input:focus-visible");
    expect(css).not.toContain("0 0 0 3px var(--bg), 0 0 0 5px var(--lime)");
    expect(css).not.toContain(".search-box:focus-within { border-color:var(--lime); box-shadow");
  });
  ...
  it("logs out from desktop, redirects to login, remains logged out after reload, and preserves data for re-login", async () => {
    ...
    await user.click(screen.getByLabelText("Sign out from desktop account menu"));
    const dialog = await screen.findByRole("dialog", { name: "Sign out of Sonora?" });
    ...
    expect(repository.sessionUser()).toBeNull();
    repository.login("listener.basic@sonora.demo", "DemoPass123!");
    expect(repository.visiblePlaylists().some((playlist) => playlist.title === "Kept local playlist")).toBe(true);
  });
  ...
  it("exposes logout from desktop, mobile account menu, and account settings", async () => {
    ...
    repository.login("listener.gold@sonora.demo", "DemoPass123!");
    render(<MemoryRouter initialEntries={["/settings"]}><App /></MemoryRouter>);
    expect(screen.getByLabelText("Sign out from desktop account menu")).toBeInTheDocument();
    expect(screen.getByLabelText("Sign out from account settings")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: /More/i }));
    expect(screen.getByLabelText("Sign out from mobile account menu")).toBeInTheDocument();
  });
  ...
  it("logs support users out without leaving staff navigation visible", async () => {
    ...
    await user.click(screen.getByLabelText("Sign out from desktop account menu"));
    await user.click(within(await screen.findByRole("dialog", { name: "Sign out of Sonora?" })).getByRole("button", { name: "Sign out" }));
    expect(await screen.findByRole("heading", { name: "Welcome back" })).toBeInTheDocument();
  ...
  it("keeps registration password bindings and maps mismatch/email/privacy errors to fields", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Birth date"), { target: { value: "1994-04-03" } });
    ...
    await user.type(screen.getByRole("textbox", { name: /Email/i }), "listener.basic@sonora.demo");
  ...
  it("registers listener and artist accounts with visibly typed passwords", async () => {
    const user = userEvent.setup();
    const listener = render(<MemoryRouter initialEntries={["/register"]}><App /></MemoryRouter>);
    await user.type(screen.getByLabelText("Display name"), "Typed Listener");
    await user.type(screen.getByLabelText("Email"), "typed-listener@example.com");
    fireEvent.change(screen.getByLabelText("Birth date"), { target: { value: "1994-04-03" } });
    await user.type(screen.getByLabelText("Password"), "LongPass123!");
    ...
    repository.logout();
    render(<MemoryRouter initialEntries={["/register/artist"]}><App /></MemoryRouter>);
    ...
    await user.type(screen.getByLabelText("Confirm password"), "ArtistPass123!");
    await user.click(screen.getByRole("checkbox"));
    await user.click(screen.getByRole("button", { name: "Create an account" }));
    expect(await screen.findByText("Popular right now")).toBeInTheDocument();
    expect(repository.sessionUser()?.artistProfile?.stageName).toBe("Typed Stage");
  });
  ...
  it("requires explicit test payment confirmation before activating a subscription", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /Confirm test payment/i }));
